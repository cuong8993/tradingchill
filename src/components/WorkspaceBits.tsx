...
export function MarketHeader({symbol,quote}:{symbol:string;quote?:Quote}){
  return <section className="market-head">
    ...
    <div className="quote">
      ...
      {quote?.extendedPrice!=null&&<small className={quote.extendedSession==='pre'?'pre':'post'}>{quote.extendedSession?.toUpperCase()} {money(quote.extendedPrice)}</small>}
    </div>
...
export function ChartCard({
  loading,error,candles,source,note,quote,indicators,alerts,volumeMA,fitSignal,logScale,onVolumeSettings,retry
}:{
  loading:boolean;
  error:string;
  candles:Candle[];
