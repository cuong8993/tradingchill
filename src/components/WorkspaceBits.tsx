...
export function MarketHeader({symbol,quote}:{symbol:string;quote?:Quote}){
  return <section className="market-head"><div><span className="instrument-avatar">{symbol.slice(0,1)}</span><div><h1>{symbol}</h1><p>Real-time workspace</p></div></div><div className="quote"><strong>{money(quote?.price)}</strong><span className={(quote?.change??0)>=0?'up':'down'}>{quote?`${quote.change>=0?'+':''}${quote.change.toFixed(2)} (${pct(quote.changePercent)})`:'-'}</span></div><div className="ohlc"><span>O <b>{money(quote?.open)}</b></span><span>H <b>{money(quote?.high)}</b></span><span>L <b>{money(quote?.low)}</b></span><span>Prev <b>{money(quote?.previousClose)}</b></span></div></section>;
}

export function ChartCard({
  loading,error,candles,source,note,currentPrice,indicators,alerts,retry
}:{
  loading:boolean;
  error:string;
  candles:Candle[];
  ...
  return <section className="chart-card">
    {loading?<div className="chart-state"><LoaderCircle className="spin"/>Loading real market data...</div>:error?<div className="chart-state"><strong>Chart unavailable</strong><span>{error}</span><button onClick={retry}>Try again</button></div>:<>
      <MarketChart candles={candles} livePrice={currentPrice} indicators={indicators} alerts={alerts}/>
